const SQL_EVENT = {
    ...
    UPDATE_EVENT: "UPDATE countryclub.EVENT SET event_name=?,e_description=?,start_date=?,end_date=?,status=?,venue_id=?,capacity=?  where event_id=?",
    DELETE_EVENT: "Update countryclub.EVENT set status='Cancelled' where event_id=?",
    GET_VENUE: "SELECT * FROM venue",
    GET_USER_EMAIL: "select email_id from user where user_id in (select user_id from event_booking)"
  };
